%210624 psignfit for contrast sensitivity using log10 to correct for many
%low points
 function [x t0 CI90]=psf_CSF1(r2, c2, s1, s2, d0)
...
result2 = psignifit(d2,options);

% figure; plotPsych(result); ylim([0 1])
% subplot(length(s2),length(s2), length(s2).*(k0-1)+k0);  plotPsych(result); ylim([0 1]);
subplot(2,4, k0);  plotPsych(result1); ylim([0 1]);

t0(k0)=10.^(result2.Fit(1));
CI90(k0,:)=10.^(result2.conf_Intervals(1,:,2));
disp([[x(k0); 95; 90; 68] [[sum(d1(:, 3)), (10.^(result2.Fit(1)))]; (10.^(squeeze(result2.conf_Intervals(1,:,:))))']]);
