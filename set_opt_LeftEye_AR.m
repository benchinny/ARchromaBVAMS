...
p4(5,1)=opL(0.8);          %R_optopower(mag)  11.3200D  , we will update this    
...
% p4=zeros(8,2);
% %if set power_disp/optopower=0 randomize.
% p4(1,:)=[9                    9];               %power_dispL_min      power_dispR_min
% p4(2,:)=[1000./75+1/0.789     12.801920]; %power_dispL          power_dispR   
% p4(3,:)=[17                   17];              %power_dispL_max      power_dispL_max                   
% p4(4,:)=[opL(0.8)             opR(0.8)];        %L_optopower(mag)_min R_optopower(mag)_min         
% p4(5,:)=[opL(1)               9.837492];          %L_optopower(mag)     R_optopower(mag)     
% p4(6,:)=[opL(1.2)             opR(1.2)];        %L_optopower(mag)_max R_optopower(mag)_max
% p4(7,:)=-3; %rotation
% fprintf('p4 initial \n');
% p4
magn=0.8;
...
avg_pow_red_left=14.132
avg_pow_green_left=13.5852
avg_pow_blue_left=12.7729
...
elseif mean_trombone_power_left<opL(1.2) && mean_trombone_power_left>opL(1.3)
    ...
    fprintf('Magnification is higher than 1.2 ,BUT Less than 1.3 (beam size is still less than 5.2 at ACL\n');
else
    fprintf('!!!!!!!!!!CAUTION: Mean trombone power  is FULLY OUTSIDE the limits !!!!!!!!!!\n');
    fprintf('!!!!!!!!!!BEAM SIZE HAS EXCEEDED 5.2 mm !!!!!!!!!!\n');
    flag=1; %set limit of 1.2 now
end
