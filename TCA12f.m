%210702 adding green focus function using fcs_grn
function [xy4 n4 y6]=TCA12f(fnm, xy0, rgb, p0, p5, window1, window2);
...
Efcs=fcs_clr(7, 2, window1, window2);
% % i3=uint8(zeros([sz 3])); i3(:,:,2)=embd(T10c{13}, uint8(255.*ones(sz))); %%setgreen optotype image
% Esz=7;
% i3=uint8(zeros([sz 3])); i3(:,:,2)=uint8(255.*embd(usz(binv(opt('E', 4)), Esz), ones(sz))); %%setgreen optotype image
