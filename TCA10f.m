function [xy4 n4 y6]=TCA10f(fnm, xy0, rgb, p0, p5, window1, window2);
...
% close all
% clc
global snt sp kntr ifi cf sz rc00 name_map l_trombone_f r_trombone_f l_opto_f r_opto_f  enable_optotunes enable_trombones zaber opto
...
%        disp(['EXP' n2s(k0) ' L_EYE DOWN' n2s(x3l) ' RIGHT' n2s(y3l) ' R_EYE DOWN' n2s(x3r) ' RIGHT' n2s(y3r) ' RES' n2s(p4)  ' ' s2(s1)]);
        disp(['EXP' n2s(k0) 'DOWN' n2s(x3r) ' RIGHT' n2s(y3r) ' RES' n2s(p4)  ' ' s2(s1)]);
        ...
        save('data\TCAtmp.mat', 'xy4', 'rgb', 'n4', 'y6');
