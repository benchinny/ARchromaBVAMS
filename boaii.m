...
function [p12 rvc wvc zzc wn]=boaii(p0, p00, bxy, ey, window1, window2);
...
bc={'white', 'red', 'green', 'blue'}; %background color

%% generate pre-stim 20/30 optotype
i00={binv(opt(L, 0))}
s00=4.34.*1.5; %b.^(w./2);%stroke size
...
for k1=1:size(p00, 1);
    ...
    %% set/start optotune
    power_dispL=p00(k1,1); power_dispR=p00(k1,2);
    powerL=p00(k1, 3); powerR=p00(k1, 4);
    magL=p00(k1, 5); magR=p00(k1, 6);
    rot=p00(k1, 7); rgd=p00(k1, 8);
    
    
    
    i33=ct4(imgf(dszf(circshift(i22, xy(1,:)), ms), sd), rgd);
...
%                 if keyCode(KbName('w'))
%                     %                      mag = 1.3;
%                     magL = magL + 0.1;%changed to 0.1 step size for new quadratic equation, 0.1 step size = 0.02D change in LCA for 8D optotune power and is 0.006D change in LCA for 12.5 D optotune power
%                     magR = magR + 0.1;%changed to 0.1 step size for new quadratic equation, 0.1 step size = 0.02D change in LCA for 8D optotune power and is 0.006D change in LCA for 12.5 D optotune power
%                 elseif keyCode(KbName('z'))
%                     magL = magL - 0.1; %0.005 for Austin %0.01 earlier
%                     magR = magR - 0.1; %0.005 for Austin %0.01 earlier
                if keyCode(KbName('s'))
...
%             end
                ...
                powerL = opto_power(magL);
                powerR = opto_power(magR);
        ...
        p11=[power_dispL power_dispR powerL powerR magL magR rot rgd];
    ...
    for ni=1:n;
        ...
        disp([ni wtt s0 s1 t]);% trial, base-power, stroke size, optotype size, time to generate optotype
    ...
    % 0.00384deg/pixels,  260.417 pixels/degree, 4.34 pixels/arcmin,  0.7234 pixels/arcsec
    yy=2.^(rslt(1)./2); zz=20.*yy./4.34; zzc(k1)=zz;
%     disp(['YOUR ACUITY SCORE IS 20/' num2str(zz)  ' ' bc{p11(8)+1} ' BACKGROUND '  num2str(p11(6)) ' MAGNIFICATION']);
    disp(['YOUR ACUITY SCORE IS 20/' num2str(zz)  ' '  num2str(p11(6)) ' MAGNIFICATION']);
    ...
    save(['data\AJR_' tmes], 'p11', 'rv', 'wv', 'yy', 'zz', 'rslt', 'cnfd');
