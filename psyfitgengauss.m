...
% function [mFit,sFit,bFit,Tfit,PCdta,PCfit,negLL] = psyfitgengauss(Xstd,Xcmp,RcmpChs,mFix,sFix,bFix,DPcrt,nIntrvl,bPLOT,xLbl,yLbl,color,shape,figh)
%
%   example call:   % FIT SUBJECT DATA... ONE-INTERVAL DATA
%                   psyfitgengauss(stdX,cmpX,R,[],[],[1],1,1,1);
%
%                   % FIT SUBJECT DATA... TWO-INTERVAL DATA
%                   psyfitgengauss(S.stdX),abs(S.cmpX),S.R == S.cmpIntrvl,[],[],[1],1,2,1);
%
%                   % FIT SIMULATED PSYCHOMETRIC DATA
%                   clear R; clear Xcmp;
%                   N = 30; Xlvl = -8:2:; Xstd = 0; Xcmp = reshape(repmat(Xlvl,[N 1]),[N.*length(Xlvl) 1]);
%                   Rlvl = binornd(N,psyfuncgengauss([],Xlvl,0,2,1,1,0)');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% mFit:     mean      of function fit (i.e. PSE)
% sFit:     sigma     of function fit 
% bFit:     beta      of function fit
% Tfit:     threshold of function fit (i.e. JND for criterion d')
% PCdta:    percent comparison chosen for the raw data
% PCfit:    percent comparison chosen for the fit at each Xcmp
