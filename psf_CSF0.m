%psignfit for contrast sensitivity
 function [x t0 CI95]=psf_CSF0(r2, c2, s1, s2, d0)
...
x=1920./(2.*s1).*s2./c1;
for k0=1:length(s2)
d0=hst1(c2(k0,:), r2(k0,:), 0); %data=d0; %save(['data' n2s(k0) ' ' tme], 'data');
...
t0(k0)=result.Fit(1);
CI95(k0,:)=result.conf_Intervals(1,:,3);
disp([[x(k0); 68; 90; 95] [sum(d0(:, 3)) result.Fit(1); squeeze(result.conf_Intervals(1,:,:))']]);
