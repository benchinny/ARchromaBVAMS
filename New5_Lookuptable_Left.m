...
opto_power=@(mag) 11.32.*((mag).^(-1.039));

%to start from a smallar beam size
mag=0.826; %13.81788 is the limiting value of dioptre for left side
%opto_power(0.825)gives 13.8245
%so limit on mag is 0.826, can't go lower than 0.826
power=opto_power(mag); % 14.27D  for mag of 0.8, and 13.8072D for a mag of 0.826
rot = -3; %at -3 degree, for left side, display shd be at 15.94
    ...
    while true
        ...
        if keyIsDown
            if keyCode(KbName('UpArrow'))
               power=power-0.2; %Reduce the power to increase the magnification
            ...
            if power>opto_power(0.826) 
               power=opto_power(0.826);
