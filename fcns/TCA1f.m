...
function bxy=TCA1f(rd, kd, bd, lineWidthPix, pixelsPerPress, window);

global ek uk dk lk rk st snt sp black white grey screenXpixels screenYpixels xCenter yCenter ifi cf
...
while exitDemo == false
    ...
    [keyIsDown,secs, keyCode] = KbCheck; ppp=pixelsPerPress(1);
    ...
    elseif keyCode(sp)
        if snt==0;
        pixelsPerPress=circshift(pixelsPerPress, 1); snt=1; % space count
        end
    elseif keyCode(lk)
        squareX = squareX - ppp; snt=0;
    elseif keyCode(rk)
        squareX = squareX + ppp; snt=0;
    %elseif keyCode(upKey)
    elseif keyCode(dk)
        squareY = squareY - ppp; snt=0;
...
%     centeredRect = CenterRectOnPointd(br, squareX, squareY);
    ...
    bxy=[squareX-xCenter+1 squareY-yCenter]
