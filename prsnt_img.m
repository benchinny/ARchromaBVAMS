...
while ext == false; %find(KeyCode)~=ek
    ...
    if kc(uk)
        chk=0;
    elseif kc(rk) & chk==0; 
        b=circshift(b,[0 1]); chk=1;
        disp(b{1});
    elseif kc(lk) & chk==0;  %find(KeyCode)==lk
        b=circshift(b,[0 -1]); chk=1;
        disp(b{1});
