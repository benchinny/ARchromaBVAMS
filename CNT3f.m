% function [i4 y2 y1 y0]=CNT2f(p0, p1, window1, window2);
%function CNTp=CNT2f(p0, p1, window1, window2);
function CNTp=CNT3f(p0, p1, window1, window2);
...
[iLf iRf]=cwin3(ct8(zeros(sz), m, c), ct8(zeros(sz), m, c), [0.25 0.25; 0.25 0.25; 1 1], rc00, window1, window2);
%cwin1(b1, b2, cf, rc00, window1, window2);
%[iLf iRf]=cwin3(imread('crs10.png'), imread('crs10.png'), cf, rc00, window1, window2);
% for k0=1:length(i3)
k0=1;
while k0<=length(i3)
...
%         
    ...
    [iLf iRf]=cwin3(flipud(ir), flipud(il), [0.25 0.25; 0.25 0.25; 1 1], rc00, window1, window2);
...
%     disp(n2s(y0));
    %disp(n2s(y2));
    disp(['TRL' n2s(k0) ' DSPRT' n2s(i5) ' RSP' n2s(y2(k0)) ])
    end
