...
function [rsp r2]=FRNG1f(p0, bxy0, bxy1, e0, s0, window1, window2)
...
while n<=ni 
    chk_rpt=0;
     if w0~=w1;
        w1=w0;
     %% TCA corrected 
     ir1= embd(circshift(e0{w1, r1, j3(1,1), j3(1,2)}    ,[j1(1,1)-j4(1,1) j1(1,2)-j4(1,2)] ),i1); %window2 LFT screen RED
     ig1= embd(circshift(e0{w1, r1, j3(2,1), j3(2,2)}    ,[j1(2,1)-j4(2,1) j1(2,2)-j4(2,2)] ),i1); %window2 LFT screen GREEN
     ib1= embd(circshift(e0{w1, r1, j3(3,1), j3(3,2)}    ,[j1(3,1)-j4(3,1) j1(3,2)-j4(3,2)] ),i1); %window2 LFT screen BLUE
     im1=cat(3, ir1, ig1, ib1);

     %% TCA UNcorrected 
     ir0= embd(circshift(e0{w1, r1, h3(1,1), h3(1,2)}    ,[h1(1,1)-h4(1,1) h1(1,2)-h4(1,2)] ),i1); %window2 LFT screen RED
     ig0= embd(circshift(e0{w1, r1, h3(2,1), h3(2,2)}    ,[h1(2,1)-h4(2,1) h1(2,2)-h4(2,2)] ),i1); %window2 LFT screen GREEN
     ib0= embd(circshift(e0{w1, r1, h3(3,1), j3(3,2)}    ,[h1(3,1)-j4(3,1) h1(3,2)-h4(3,2)] ),i1); %window2 LFT screen BLUE
     ...
     if rand <0.5
         ...
         rsd=0; %fringe appears on the left side
         r2(n)='L';
     end
         im3=unit8(im2);
       cwin1(im3, im3, cf, rc00, window1, window2); 
     
     pause(dly);
% 
     cwin1(uint8(255.*ones([sz 3])), uint8(255.*ones([sz 3])), cf, rc00, window1, window2);
     ...
     [secs, kc, deltaSecs] = KbStrokeWait;
        ...
        elseif kc(KbName('RightArrow')) 
            drc(n)=1;
             %disp([r2(n) n2s(drc(n)==rsd) ' S' n2s(w0)])
             %rsp(n)=(drc(n)==rsd); 
        ...
        if w0<1; w0=1; elseif w0>length(s0); w0=length(s0); end
             disp(['N' n2s(n) ' ' r2(n) n2s(drc(n)==rsd) ' S' n2s(w0)])
             rsp(n)=(drc(n)==rsd); 

        
     end
     
     save(['data\FRNGtmp'], 'bxy0','bxy1', 'rsp', 'rsd', 'im2');
