%%211119 AFC5f autorefractor with 
function [v1 t3 dgs]=AFC7f(im_path, v0, sr, window1, window2)
...
global ek uk wk dk zk lk ak rk sk st black white grey screenXpixels screenYpixels xCenter yCenter ifi zbr cf rc00 name_map l_trombone_f r_trombone_f l_opto_f r_opto_f  enable_optotunes enable_trombones zaber opto ey log
    ...
    power_dispR=14+2+sr(2); %starting display power
    power_dispL=14.4+2+sr(1); %starting display power
...
dgs=atand(ipd.*v0)./2-3; %degrees to rotate
dgs(dgs<=-3)=-3;
...
%            if (k0==25) | (k0==50); KbWait([], 2); end
%          if (k0==3) | (k0==7); KbWait([], 2); end
          ...
          if scene.enable_tcp; send_tcp0(scene, 1); end
          t0(k0,:)=clock;
