%% Set varichrome to desired position with magnification parameter.
function vrcm(p0)
...
%mag=p0(3);
rot=p0(3,1);
off=p0(4,1); %turn off system =1

%% turn of initialize sys
a = instrfind();
...
power_L=mag_to_d(mag_L);
power_R=mag_to_d(mag_R);


if enable_optotunes
    opto(name_map('l_disp')).control.setFocalPower(power_disp_L);
    opto(name_map('l_t_near')).control.setFocalPower(power_L);
    opto(name_map('l_t_far')).control.setFocalPower(power_L);
    opto(name_map('r_disp')).control.setFocalPower(power_disp_R);
    opto(name_map('r_t_near')).control.setFocalPower(power_R);
    opto(name_map('r_t_far')).control.setFocalPower(power_R);
end
...
%     mag_R=p0(2,2);
    ...
    zaber(name_map('l_trombone')).move(l_trombone_f(power_L));
    zaber(name_map('r_trombone')).move(r_trombone_f(power_R));
    zaber(name_map('rotation')).control.waitforidle();
    zaber(name_map('l_trombone')).control.waitforidle();
    zaber(name_map('r_trombone')).control.waitforidle();
end
...
% % %                      mag = 1.3;
% %                      mag = mag + 0.1;%changed to 0.1 step size for new quadratic equation, 0.1 step size = 0.02D change in LCA for 8D optotune power and is 0.006D change in LCA for 12.5 D optotune power
% %                 elseif keyCode(KbName('DownArrow'))
% %                      mag = mag - 0.1; %0.005 for Austin %0.01 earlier
