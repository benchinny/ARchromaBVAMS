%210628 psignfit for contrast sensitivity using log10 to correct for many
%low points
 function [x t0 CI90]=psf_CSF2(r2, c2, s1, s2, d0)
...
x=1920./(2.*s1).*s2./c1;
for k0=1:length(s2)
d1=hst1(c2(k0,:), r2(k0,:), 0); %data=d0; %save(['data' n2s(k0) ' ' tme], 'data');
d2=d1; d2(:,1)= log10(d2(:,1));
%result1 = psignifit(d1,options);
options.fixedPars = [NaN,NaN,0,0.5,NaN]; 
result = psignifit(d2,options);

% figure; plotPsych(result); ylim([0 1])
% subplot(length(s2),length(s2), length(s2).*(k0-1)+k0);  plotPsych(result); ylim([0 1]);
subplot(2,4, k0);  plotPsych(result); ylim([0 1]);

t0(k0)=10.^(result.Fit(1));
CI90(k0,:)=10.^(result.conf_Intervals(1,:,2));
% disp([[x(k0); 95; 90; 68] [[sum(d1(:, 3)), (10.^(result.Fit(1)))]; (10.^(squeeze(result.conf_Intervals(1,:,:))))']]);

disp([[x(k0); 95; 90; 68] [[sum(d1(:, 3)), (10.^(result.Fit(1))), (10.^(result.Fit(1))) result.Fit(1)]; squeeze(result.conf_Intervals(1,1,:)) (10.^(squeeze(result.conf_Intervals(1,:,:))))' squeeze(result.conf_Intervals(1,2,:))]]);
