...
function [Full Half]=LCA16f(im_path, p0, window1, window2)
%function [p1   p2   p3   p4   p5   p6   p7   p8   p9   p10]=LCA15f(im_path, p0, window1, window2)
        %[a10, a11, a12, a13, a14, a15, a16, a17, a18, a19]
%p1(k0,:)=[power_dispL power_dispR powerL powerR rot];
global ek uk wk dk zk lk ak rk sk st black white grey screenXpixels screenYpixels xCenter yCenter ifi zbr cf rc00 name_map l_trombone_f r_trombone_f l_opto_f r_opto_f  enable_optotunes enable_trombones zaber opto ey
...
%     disp(['\n WAS RETICLE UPDATED TO THE ABOVE IN LABVIEW?: mag=' n2s2((p0(5)))]); %was input before for some reason
%     
...
%         wn=cwin0(img{n4(k0)}, ey, cf, rc00, window1, window2);
       ...
       [iLf iRf]=cwin3(img{n4(k0)}, img{n4(k0)} , cf, rc00, window2, window1);   
    ...
    Full.p1=p1; Half.p1=p1;
    %p1=a10 [power_dispL power_dispR powerL powerR rot n4(k0)];
    for k0=1:length(im_path)
        p2(k0,:)=mean(p1(p1(:,6)==k0,:),1); %a11
        p3(k0,:)=std(p1(p1(:,6)==k0,:), 0, 1); %CAUTION:also p3 is being used further!!! %a12
    end
    Full.p2=p2; Half.p2=p2; Full.p3=p3; Half.p3=p3;
    ...
    p5(1,:)=p4(1,:)-(0.2174.*p0(5)+0.0818); %red-blue
    p5(2,:)=p4(2,:)-(0.1231.*p0(5)+0.0297); %green-blue
    p5(3,:)=p4(3,:)-(0.0943.*p0(5)+0.0521); %red-green
    %p5=a14 Chromatic difference of refrction
    p50=p5./2; %half LCA correction
    Full.p5=p5; Half.p5=p50;
    ...
    %p6=a15
    %half LCA calc
    p60(:,1)=([9.2813 7.1652 6.8831].*(p50(:,1)').^[-0.403 -0.414 -0.39])'; %left side
    p60(:,2)=([9.1613 7.2167 6.9543].*(p50(:,2)').^[-0.371 -0.382 -0.36])' %Right side
    fprintf('Mean Trombone powers');
    Full.p6=p6; Half.p6=p60;
    
    
    %calculate mean
    p7=mean(p6,1); %a16
    p70=mean(p60,1); %HalfLCA
    Full.p7=p7; Half.p7=p70;
    %a17
    p8(1)=bnd(p7(1), powerL_min, powerL_max);
    p8(2)=bnd(p7(2), powerR_min, powerR_max);
    
    p80(1)=bnd(p70(1), powerL_min, powerL_max);
    p80(2)=bnd(p70(2), powerR_min, powerR_max);
    
    Full.p8=p8; Half.p8=p80;
%     if ey(1)=='R'
...
%     end
    ...
    p90=p9;
    p90(5,:)=p80; %trombone power
    Full.p9=p9; Half.p9=p90;
    %p10=p90;
