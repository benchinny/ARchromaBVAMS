...
%     d2=d1; d2(:,1)=s0(d2(:,1))./c1; %MAR Acuity
    ...
    % % data=[bns' v1' v0'];
    rLOG = psignifit(d3,options);
    rMAR = psignifit(d2,options);
    

    %figure; plotPsych(result);
    figure; plotPsych(rMAR,plotOptions);
%     rslt=result.Fit; %disp(n2s2(rslt))
%     cnfd=result.conf_Intervals;
    [r2LOG CILOG]=getThreshold(rLOG, 0.781);%threshold at 78.1% r3=confidence interval
        [r2MAR CIMAR]=getThreshold(rMAR, 0.781);%threshold at 78.1% r3=confidence interval
...
%    if y0(k0)==1;
        %r1=[r1 r2];%rslt(1)];
        title(['ACMD' n2s(y0)  ' VA@78.1%= ' n2s2(r2LOG) 'LogMAR ' n2s2(r2MAR) 'MAR']); %disp(r2); disp(CI);
        r1=[[y0; 95; 90; 68] [[sum(d1(:,3)) sum(d1(:,3)) r2MAR r2LOG]; CILOG(:,1), CIMAR, CILOG(:,2)]];
%    elseif y0(k0)==0;
        %r0=[r0 r2];%rslt(1)];
