...
for k0=1:size(w2,1)
    ...
    rslt=result.Fit;
    cnfd=result.conf_Intervals; %get confidence intervals
    [r2 CI]=getThreshold(result, 0.781)%threshold at 78.1%

    if y0(k0)==1;
        r1=[r1 r2];%rslt(1)];
        %title(['EXP#' num2str(k0+4) ' TCA1 Threshold' n2s(rslt(1))  ' VA=20/'  n2s(20.*rslt(1)./c1)]);
        title(['EXP#' num2str(k0) ' TCA1 Threshold' n2s(rslt(1))  ' VA=20/'  n2s(20.*rslt(1)./c1)]); disp(20.*r2./c1); disp(20.*CI./c1);
        %title(['EXP#' n2s(k0) ' TCA1 @78.1%=' n2s2(r2)  ' VA=20/'  n2s2(20.*r2./c1) ' p=' n2s2(cnfd)]);


    else
        r0=[r0 r2];%rslt(1)];
        %title(['EXP#' num2str(k0+4) ' TCA0 Threshlod'  n2s(rslt(1))  ' VA=20/' n2s(20.*rslt(1)./c1)]);
        title(['EXP#' num2str(k0) ' TCA0 Threshlod'  n2s(rslt(1))  ' VA=20/' n2s(20.*rslt(1)./c1)]); disp(20.*r2./c1); disp(20.*CI./c1);
        %title(['EXP#' n2s(k0) ' TCA0 @78.1%=' n2s2(r2)  ' VA=20/'  n2s2(20.*r2./c1) ' p=' n2s2(cnfd)]);
